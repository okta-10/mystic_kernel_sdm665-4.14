...
{
	pr_debug(" s_ctrl->sensordata->camera_type = %d\n",
		 s_ctrl->sensordata->sensor_info->position);
...
void msm_sensor_set_sesnor_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if ((!strcmp("ginkgo_s5kgm1_sunny_i", s_ctrl->sensordata->sensor_name)) ||
		...
		rc = msm_sensor_get_sensor_id_ginkgo_s5kgm1(s_ctrl,
	...
	if ((!strcmp("ginkgo_ov13855_sunny_i", s_ctrl->sensordata->sensor_name)) ||
		...
		rc = msm_sensor_get_sensor_id_ginkgo_ov13855(s_ctrl,
							     sensor_fusion_id_tmp);
		if (rc < 0)
			pr_err("%s:%d litao read sensor %s fusion id failed\n",
			       __func__, __LINE__,
	...
	struct msm_sensor_info_t *probed_info, char *entity_name)
{
	...
	if (s_ctrl->is_probe_succeed == 1) {
		...
		if (slave_info->sensor_id_info.sensor_id ==
			...
			s_ctrl->sensordata->cam_slave_info->vendor_id_info
			.vendor_id)) {
